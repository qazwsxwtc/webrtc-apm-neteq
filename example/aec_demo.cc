...
int main() {
    ...
    printf("=== AEC Demo: Echo Cancellation ===\n");
    printf("Sample rate    : %d Hz\n", kSampleRate);
    printf("Frame size     : %zu samples (10 ms)\n", kFrameSamples);
    printf("Echo latency   : %d ms (simulated via ring buffer)\n", kEchoDelayMs);
    printf("Far-end signal : 440 Hz sine\n\n");
    ...
    printf("%-6s %-10s %-12s %-12s %-12s %-10s\n",
           "Frame", "RenderRMS", "CaptureRMS", "AecOutRMS", "ERLE_dB", "DelayMs");
    printf("-----------------------------------------------------------\n");

    for (int frame = 0; frame < 80; ++frame) {
        FillSine(render.data(), kFrameSamples, 440.0f, 0.3f, frame);
        ...
        for (size_t i = 0; i < kFrameSamples; ++i) {
            ...
            float noise = static_cast<float>(std::rand()) / RAND_MAX * 0.02f;
            capture[i] = delayed_render * 0.8f + noise;
        }
        ...
        double erle = stats.echo_return_loss_enhancement.value_or(-1.0);
        int delay = stats.delay_ms.value_or(-1);

        printf("%-6d %-10.4f %-12.4f %-12.4f %-12.1f %-10d\n",
               frame, render_rms, capture_rms, out_rms, erle, delay);
    }
